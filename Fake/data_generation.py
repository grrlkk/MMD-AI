...
from DB.opensearch import OpenSearchDB  # ← 리트리버 안 씀(인증 래퍼 그대로 사용)
...
def _random_sample_by_category_code(code: int, size: int, seed: Optional[int] = None) -> dict:
    seed_val = seed if seed is not None else random.randint(1, 10_000_000)
    # job_category 필드가 numeric 매핑이라고 가정
    return {
        "size": size,
        "query": {
            "function_score": {
                "query": {"bool": {"filter": [{"term": {"job_category": code}}]}},
...
def collect_candidates_by_mapping(mapping_path: str,
    ...
    db = OpenSearchDB()
    pool: Dict[str, Dict] = {}
    total_target = min(max_total, per_code * sum(len(v) for v in grouped.values()))

    print(f"매핑 로드 완료: 총 소분류 {len(code2role)}개 코드")
    for big_code, codes in grouped.items():
        for code in codes:
            try:
                body = _random_sample_by_category_code(code, size=per_code, seed=seed)
                resp = db.search(body, size=per_code)
                hits = (resp or {}).get("hits", {}).get("hits", [])
                for h in hits:
                    _id = h.get("_id", "")
                    src = h.get("_source", {}) or {}
                    # 직무명(일반화용) 힌트를 함께 보관
                    src["_mapped_role_name"] = code2role.get(code, "기타")
                    src["_mapped_role_code"] = code
                ...
                print(f"[경고] 코드 {code} 샘플링 중 오류: {e}")
        if len(pool) >= total_target:
            break

    print(f"\n총 {len(pool)}개의 고유 후보 문서를 수집했습니다. (카테고리 코드 기반 랜덤)")
    return pool
...
def main():
    NUM_SAMPLES = 200
    PER_CODE = 5                      # 소분류 코드별 뽑는 개수
    FILE_NAME = "retriever_sample_data.json"
